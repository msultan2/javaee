...
public:
     ...
     */
    virtual ::TTime_t getLocalTime() const;

    /**
     * @brief Get current UTC time
     */
    ::TTime_t getUniversalTime() const;

    /**
     * @brief Get current steady time.
     * A steady clock a clock that will always return a later time when it is accessed later.
     * So no time change e.g. via NTP will have impact on it
     */
    ::TSteadyTimePoint getSteadyTime() const;
