...
Model::Model(View::OutStationConfigurationParameters& coreConfiguration)
:
m_pDataContainer(),
m_pQueueDetector(),
m_pIniConfiguration(),
...
m_pActiveGSMModemSshMonitor(),
m_pSignalLevelProcessor()
{
    View::OutStationMainFrame* pMainFrame = View::View::getMainFrame();

    m_pDataContainer = boost::shared_ptr<DataContainer>(new DataContainer());
    m_pDataContainer->addObserver(View::View::getInstancePtr());

    uint64_t lastUsedBlueToothDevice = m_coreConfiguration.getLastUsedBlueToothDevice();
    m_pDataContainer->getLocalDeviceRecord().address = lastUsedBlueToothDevice;
...
#error Operating System not supported
    ...
    if (pMainFrame != 0)
    {
        pMainFrame->setup(m_pQueueDetector);
    }
    //else do nothing
    ...
    if (pMainFrame != 0)
    {
        pMainFrame->setup(m_pInStationReverseSSHConnector);
    }
    //else do nothing
    ...
    if (isValid() && m_pIniConfiguration->isValid())
    {
        _applyNewIniConfiguration(*m_pIniConfiguration, true); //true=isConstructing
    }
    //else do nothing
}
...
bool Model::construct()
{
    if (m_instancePtr == 0)
    {
        m_instancePtr = new Model(*View::OutStationConfigurationParameters::getInstancePtr());
    }
    //else do nothing
...
void Model::applyNewIniConfiguration(const IniConfiguration& iniConfiguration)
{
    getInstancePtr()->_applyNewIniConfiguration(iniConfiguration, false);
}

void Model::_applyNewIniConfiguration(const IniConfiguration& iniConfiguration, const bool isConstructing)
{
    if (!isConstructing)
    {
...
#ifdef __linux
            sighandler(SIGQUIT, ePROGRAM_RESTART_REQUIRED);
#else
            sighandler(SIGTERM, ePROGRAM_RESTART_REQUIRED);
#endif
...
boost::shared_ptr<DataContainer> Model::getDataContainer()
{
    return getInstancePtr()->m_pDataContainer;
}

boost::shared_ptr<BlueTooth::DeviceDiscoverer> Model::getDeviceDiscoverer()
{
    return getInstancePtr()->m_pDeviceDiscoverer;
}
...
bool Model::_constructAndStartAllInstationClientClasses()
{
    ...
    if (m_pCongestionReportingActiveTCPClient != 0)
    {
        View::View::getInstancePtr()->notifyOfStateChange(
            m_pCongestionReportingActiveTCPClient.get(),
...
void Model::_createAndStartInstationRetrieveConfigurationClasses()
{
    ...
    m_pRetrieveConfigurationActiveTCPClient = boost::shared_ptr<ActiveBoostAsioTCPClient>(
        new ActiveBoostAsioTCPClient(RETRIEVE_CONFIGURATION_CLIENT_IDENTIFIER));
    ...
    m_pRetrieveConfigurationClient->addObserver(&*m_pInStationReverseSSHConnector);
    ...
    if (pMainFrame != 0)
    {
        pMainFrame->setup(m_pRetrieveConfigurationClient);
    }
    //else do nothing
...
bool Model::_setRetrieveConfigurationConnectionParameters()
{
    ...
    if (!remoteAddressOk)
    {
        Logger::log(LOG_LEVEL_DEBUG1, "Invalid host part of retrieve configuration address");
    }
    //else do nothing
    ...
    if (!remotePortOk)
    {
        Logger::log(LOG_LEVEL_DEBUG1, "Invalid port of retrieve configuration address");
    }
    //else do nothing
    ...
    boost::shared_ptr<ActiveTask>& pActiveHttpClientTask)
{
    pActiveTCPClient = boost::shared_ptr<ActiveBoostAsioTCPClient>(
        new ActiveBoostAsioTCPClient(identifier));
...
void Model::_constructInStationReporter()
{
    ...
    if (m_pActiveGSMModemSshMonitor != 0)
    {
        m_pActiveGSMModemSshMonitor->addObserver(&*m_pInStationReporter);
    }
    //else do nothing
...
void Model::_destructInStationReporter()
{
    ...
    if (m_pActiveGSMModemSshMonitor != 0)
    {
        m_pActiveGSMModemSshMonitor->removeObserver(&*m_pInStationReporter);
    }
    //else do nothing
...
bool Model::startGSMModemSSHMonitor()
{
    return getInstancePtr()->_startGSMModemSSHMonitor();
}

bool Model::_startGSMModemSSHMonitor()
{
    bool result = false;
    if (
        ...
        )
    {
        ...
        m_pActiveGSMModemSshMonitor = boost::shared_ptr<ActiveGSMModemSshMonitor>(
            new ActiveGSMModemSshMonitor(
                m_coreConfiguration,
                *m_pSignalLevelProcessor,
                &m_pDataContainer->getGSMModemUnableToConnectFault()
                ));

        m_pActiveGSMModemSshMonitor->setup(
            m_coreConfiguration.getGSMModemConnectionAddress(),
            m_coreConfiguration.getGSMModemConnectionPort(),
        ...
        m_pActiveGSMModemSshMonitor->start();

        //if (pMainFrame != 0)
        //{
        //    pMainFrame->setup(m_pActiveGSMModemSshMonitor);
        //}
        ////else do nothing
...
void Model::stopGSMModemSSHMonitor()
{
    getInstancePtr()->_stopGSMModemSSHMonitor();
}

void Model::_stopGSMModemSSHMonitor()
{
    if (m_pActiveGSMModemSshMonitor != 0)
    {
        m_pActiveGSMModemSshMonitor->stop();
        m_pActiveGSMModemSshMonitor->notifyObservers();
        m_pActiveGSMModemSshMonitor->removeAllObservers();
        m_pActiveGSMModemSshMonitor->shutdownThread(MODULE_NAME);
    }
    //else do nothing

    m_pActiveGSMModemSshMonitor.reset();
...
void Model::_applyQueueDetectionAlgorithmParameters(const IniConfiguration& iniConfiguration)
{
    ...
    if (m_coreConfiguration.getMajorCoreConfigurationVersion() < 4)
    {
        found = found && iniConfiguration.getValueInt64(eINQUIRY_CYCLE_PERIOD, inquiryDurationInSeconds);
        found = found && iniConfiguration.getValueInt64(eFREE_FLOW_SPEED_CYCLES_THRESHOLD, freeFlowBinThresholdInSeconds);
        found = found && iniConfiguration.getValueInt64(eMODERATE_SPEED_CYCLES_THRESHOLD, moderateFlowBinThresholdInSeconds);
        found = found && iniConfiguration.getValueInt64(eSLOW_SPEED_CYCLES_THRESHOLD, slowFlowBinThresholdInSeconds);
        found = found && iniConfiguration.getValueInt64(eVERY_SLOW_SPEED_CYCLES_THRESHOLD, verySlowFlowBinThresholdInSeconds);
        ...
        found = found && iniConfiguration.getValueInt64(eQUEUE_DETECT_THRESHOLD, queueDetectionThresholdNumber);
        found = found && iniConfiguration.getValueInt64(eQUEUE_CLEARANCE_THRESHOLD_DETECTION_NUMBER, queueClearanceThresholdNumber);
    }
    else
    {
        found = found && iniConfiguration.getValueInt64(eINQUIRY_CYCLE_DURATION_IN_SECONDS, inquiryDurationInSeconds);
        if (inquiryDurationInSeconds > 0)
            inquiryDurationInSeconds = 1; //which means - do not perform scanning
        found = found && iniConfiguration.getValueInt64(eFREE_FLOW_BIN_THRESHOLD_IN_SECONDS, freeFlowBinThresholdInSeconds);
        found = found && iniConfiguration.getValueInt64(eMODERATE_FLOW_BIN_THRESHOLD_IN_SECONDS, moderateFlowBinThresholdInSeconds);
        found = found && iniConfiguration.getValueInt64(eSLOW_FLOW_BIN_THRESHOLD_IN_SECONDS, slowFlowBinThresholdInSeconds);
        found = found && iniConfiguration.getValueInt64(eVERY_SLOW_FLOW_BIN_THRESHOLD_IN_SECONDS, verySlowFlowBinThresholdInSeconds);
        found = found && iniConfiguration.getValueInt64(eABSENCE_THRESHOLD_IN_SECONDS, absenceThresholdInSeconds);
        found = found && iniConfiguration.getValueInt64(eQUEUE_DETECT_THRESHOLD, queueDetectionThresholdNumber);
        found = found && iniConfiguration.getValueInt64(eQUEUE_CLEARANCE_THRESHOLD, queueClearanceThresholdNumber);
        found = found && iniConfiguration.getValueInt64(eQUEUE_DETECTION_STARTUP_INTERVAL_IN_SECONDS, queueDetectionStartupIntervalInSeconds);
    }
    found = found && iniConfiguration.getValueString(eQUEUE_ALERT_THRESHOLD_BIN, queueAlertThresholdBinString);
    ...
    {
        Logger::logError("Some parameters in the functional configuration file"
...
void Model::_applyPeriodicReportParameters(const IniConfiguration& iniConfiguration)
{
    if (m_coreConfiguration.getMajorCoreConfigurationVersion() >= 4)
    {
        ...
        else
        {
            if (m_pPeriodicallySendStatusReportTask != 0)
            {
                m_pPeriodicallySendStatusReportTask->start(0); //stop sending requests
            }
            //else do nothing
        }
        ...
        else
        {
            if (m_pPeriodicallySendStatisticsReportTask != 0)
            {
                m_pPeriodicallySendStatisticsReportTask->start(0); //stop sending requests
            }
            //else do nothing
        }
        ...
        else
        {
            if (m_pPeriodicallySendCongestionReportTask != 0)
            {
                m_pPeriodicallySendCongestionReportTask->start(0); //stop sending requests
            }
            //else do nothing
        }
    }
...
void Model::_applyGSMModemSignalLevelParameters(const IniConfiguration& iniConfiguration)
{
    ...
    if (ok)
    {
        m_pActiveGSMModemSshMonitor->setupSignalLevelSamplingPeriod(
    ...
    {
        Logger::logError("Invalid settingsCollectionInterval. Using default value");
    }
}
...
{
    assert(m_pRetrieveConfigurationTask != 0);
    ...
    if (ok)
    {
        m_pRetrieveConfigurationTask->setup(static_cast<long>(sendIntervalInSeconds));
    }
    else
    {
        Logger::logError("Invalid settingsCollectionInterval. Using default value");
