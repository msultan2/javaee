...
public:
    ...
    typedef void (THookFunction)(void* );

    /**
     * @brief Shutdown the thread.
     * @param requestorName the name of the requesting party (used for debugging)
     * @param pHookFunction hook function to be called to accelerate shutdown
     */
    void shutdownThread(
...
private:
    ...
    ActiveObject(const ActiveObject& rhs);
    //! copy assignment operator
    ActiveObject& operator=(const ActiveObject& rhs);
...
#endif
    mutable boost::mutex m_globalMutex;
