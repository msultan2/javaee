...
public:
     ...
     * to be executedsetConnectingTimeout
     */
    ...
    /**
     * @brief Set the value of backoff time (used when something goes wrong)
     * @param timeInSeconds time value in seconds
     */
    void setBackoffTime(const unsigned int timeInSeconds);
...
private:
    ...
    // Default constructor, copy constructor and assignment operator not defined
    ActiveBoostAsioTCPClient();
    ActiveBoostAsioTCPClient(const ActiveBoostAsioTCPClient& rhs);
    ActiveBoostAsioTCPClient& operator=(const ActiveBoostAsioTCPClient& rhs);
