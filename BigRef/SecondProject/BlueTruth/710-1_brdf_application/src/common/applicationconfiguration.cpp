...
namespace
{
    const tchar DEFAULT_USER_DIRECTORY[] = _T("Brdf");

    const tchar BRDF_FALLBACK_TEMP_DIRECTORY_NAME[] = _T("Temp");

    const tchar BRDF_INSTALL_DIR_KEY_LOCATION[] = _T("SOFTWARE\\Simulation Systems Ltd\\Brdf");
    const tchar BRDF_INSTALL_DIR_VALUE_NAME[] = _T("InstallPath");
    const tchar BRDF_WORKING_DIR_VALUE_NAME[] = _T("DataDirectory");
...
void ApplicationConfiguration::evaluateUserDataDirectory()
{
    ...
    {
        //Probably the BRDF is running from a local directory (e.g. during program development)
        //do nothing
    }
    ...
    else
    {
            ...
            )
        {
            ...
            //Modify user directory for use by the BRDF
...
void ApplicationConfiguration::evaluateTemporaryDirectory()
{
        ...
        )
    {
        ...
        if (fallBackToLocalTempDirectory)
        {
            ...
            m_temporaryDirectory.append(BRDF_FALLBACK_TEMP_DIRECTORY_NAME);
