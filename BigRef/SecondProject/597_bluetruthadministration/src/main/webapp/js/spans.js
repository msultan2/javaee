var canUpdateJourneyTimeLimit = false;
...
function create_span_table(){
    ...
    function(data){
        ...
        for(var i = 0; i < data.data.roles.length; i++) {
            var user_role = data.data.roles[i];
            if(user_role == "journey_configuration") {
                canUpdateJourneyTimeLimit = true;
                break;
            }
...
function create_span_datatable_config(filter_div){
    var datatable_config = common_datatable_configuration();
        ...
        null,
        null,
        null,                                           // 08 (Journey Time Limit)
        {
            "mDataProp":function(source, type, val){    // 09 (Logical Group)
                return source[9].replace(/\] \[/g, "] <br/> [");
            }
        },
        {
            "mDataProp":function(source, type, val){    // 10 (View Data)
        ...
        {
            "mDataProp":function(source, type, val){    // 11 (Select)
    ...
    datatable_config.fnDrawCallback = function(){
        ...
        $('td.editablenumeric', datatables['span-table'].fnGetNodes()).editable('SpanManager', jeditable_config_numeric);
        if(canUpdateJourneyTimeLimit) {
