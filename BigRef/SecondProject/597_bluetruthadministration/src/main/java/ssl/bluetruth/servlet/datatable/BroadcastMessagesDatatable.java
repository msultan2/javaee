 ...
 */
public class BroadcastMessagesDatatable extends AbstractDatatable {

    private static final Logger LOGGER = LogManager.getLogger(BroadcastMessagesDatatable.class);
    private static final String TABLE_NAME = "broadcast_message "
            + "JOIN ( "
            + "   SELECT broadcast_message_logical_group.message_id, "
            + "   ('['::text || array_to_string(array_agg(DISTINCT broadcast_message_logical_group.logical_group), '] ['::text)) || ']'::text AS logical_groups "
            + "   FROM broadcast_message_logical_group "
            + "	  JOIN instation_user_logical_group ON "
            + "	  broadcast_message_logical_group.logical_group = instation_user_logical_group.logical_group_name "
            + "	  JOIN instation_user ON "
            + "	  instation_user.username = instation_user_logical_group.username "
            + "	  AND instation_user.username = '%s'"
            + "	  GROUP BY broadcast_message_logical_group.message_id"
            + ") available ON available.message_id = broadcast_message.message_id";
    ...
    private static final String fileName = "broadcast_messages.csv";
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        columns.add(new DataTableColumnDef("Message Id", "broadcast_message.message_id", ColumnType.INTEGER));
        columns.add(new DataTableColumnDef("Title", "broadcast_message.title", ColumnType.STRING));
        columns.add(new DataTableColumnDef("Description", "broadcast_message.message", ColumnType.STRING));
