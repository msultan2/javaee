 ...
 */
public class DetectorEngineerNotesDatatable extends AbstractDatatable{

     private static final Logger LOGGER = LogManager.getLogger(DetectorEngineerNotesDatatable.class);
     private static final String TABLE_NAME = "detector_engineer_notes "
            + "JOIN instation_user ON instation_user.username = detector_engineer_notes.author "
            + "JOIN ( "
            ...
            + ") available ON available.detector_id = detector_engineer_notes.detector_id";

    private final String DETECTOR_ID_REQUEST = "detector_id";
    private final String DETECTOR_ID_DB = "detector_engineer_notes.detector_id";
    private final JsonResponseProcessor responseProcessor = new JsonResponseProcessor();
    private static final String fileName = "detector_engineer_notes.csv";
    ...
    protected List<DataTableColumnDef> getColumns(HttpServletRequest request) {
        ...
        columns.add(new DataTableColumnDef("Note Id", "detector_engineer_notes.note_id", ColumnType.INTEGER));
        columns.add(new DataTableColumnDef("Description", "detector_engineer_notes.description", ColumnType.STRING));
        columns.add(new DataTableColumnDef("Author", "instation_user.full_name", ColumnType.STRING));
        columns.add(new DataTableColumnDef("Added Timestamp", "detector_engineer_notes.added_timestamp AT TIME ZONE '" + timezone + "'", ColumnType.DATE));
        return columns;
    }
