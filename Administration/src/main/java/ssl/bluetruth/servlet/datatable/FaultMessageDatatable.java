 ...
 * @author liban
 */
public class FaultMessageDatatable extends AbstractDatatable {

    private static final Logger LOGGER = LogManager.getLogger(FaultMessageDatatable.class);
    private static final String TABLE_NAME = "fault_report "
            ...
            + "JOIN fault_message ON fault_message.report_id = fault_report.report_id AND "
            + "fault_message.report_id = %d";
    private final JsonResponseProcessor responseProcessor = new JsonResponseProcessor();
    private static final String fileName = "fault_messages.csv";
    ...
    protected String getTableName(HttpServletRequest request) {
        ...
        return String.format(TABLE_NAME, username,reportId);
    }
    ...
    protected List<DataTableColumnDef> getColumns(HttpServletRequest request) {
        ...
        tableColumns.add(new DataTableColumnDef("Fault", "fault_message.code", ColumnType.INTEGER));
        tableColumns.add(new DataTableColumnDef("Status", "fault_message.status", ColumnType.INTEGER));
        tableColumns.add(new DataTableColumnDef("Time", "fault_message.time AT TIME ZONE '" + timezone + "'", ColumnType.DATE));
